...
const COPILOT_MODES_DATA = {
  Ask: {
    heroHtml: `
      ...
      <div class="text-slate-300 font-medium pt-0.5">What would you like to do today?</div>
    `,
    badge: "SRE Intelligence",
    suggestedTitle: "Suggested for this page",
    suggestedIcon: "🎯",
    cards: [
      { id: "whats_happening_today", icon: "📊", iconColor: "text-teal-400", title: "What's happening today?" },
      { id: "find_failed_payments", icon: "🔍", iconColor: "text-sky-400", title: "Find failed payments" },
      { id: "show_revenue_at_risk", icon: "⚠️", iconColor: "text-amber-400", title: "Show revenue at risk" },
      { id: "analyze_recovery_rate", icon: "📈", iconColor: "text-cyan-400", title: "Analyze recovery rate" },
      { id: "explain_this_metric", icon: "💡", iconColor: "text-yellow-400", title: "Explain this metric" },
      { id: "start_guided_tour", icon: "▶️", iconColor: "text-blue-400", title: "Start guided tour" }
    ]
  },
  Investigate: {
    heroHtml: `
      <div class="font-bold text-sky-400 flex items-center gap-1.5 pb-0.5">
      ...
      <div class="space-y-1 pl-1 text-[11px] pt-1">
        <div>• <strong class="text-rose-400">HDFC 504 Timeout Anomaly</strong>: Error rate 14.8% due to CBS queue saturation.</div>
        <div>• <strong class="text-emerald-400">Weibull Adaptive Shift</strong>: Fast-Loop retries shifted to <span class="font-mono text-cyan-300">T+45m</span> peak.</div>
        <div>• <strong class="text-amber-400">B2B Voice Pipeline</strong>: 2 high-ticket corporate invoices transitioned to PTP negotiation.</div>
      </div>
      <div class="text-slate-400 text-[10.5px] italic pt-1">Select a diagnostic target below or enter a Payment ID in the search bar:</div>
    `,
    badge: "Investigation Desk",
    suggestedTitle: "SRE Diagnostic Actions",
    suggestedIcon: "🔍",
    cards: [
      { id: "find_failed_payments", icon: "⚡", iconColor: "text-rose-400", title: "Diagnose HDFC 504 Outage" },
      { id: "trace_pay_hdfc", icon: "🔍", iconColor: "text-sky-400", title: "Trace pay_hdfc_99812" },
      { id: "inspect_cas_mutex", icon: "🛡️", iconColor: "text-emerald-400", title: "Inspect CAS Mutex Locks" },
      { id: "whats_happening_today", icon: "📊", iconColor: "text-teal-400", title: "Bank Switch Latency Radar" },
      { id: "cfo_dispute_queue", icon: "📑", iconColor: "text-amber-400", title: "B2B CFO Dispute Queue" },
      { id: "storm_50_threads", icon: "🔄", iconColor: "text-purple-400", title: "Run 50-Thread Storm Test" }
    ]
  },
  Explain: {
    heroHtml: `
      <div class="font-bold text-yellow-400 flex items-center gap-1.5 pb-0.5">
      ...
      <div class="space-y-1 pl-1 text-[11px] pt-1">
        <div>• <strong class="text-cyan-300">Weibull Hazard ($h(t)$)</strong>: Continuous parametric recovery curve vs. bank half-life.</div>
        <div>• <strong class="text-emerald-400">CAS Mutex Idempotency</strong>: Atomic compare-and-swap preventing duplicate charges.</div>
        <div>• <strong class="text-amber-300">Deterministic Clamping</strong>: Hard statutory caps: discount &le; min(10%, ₹500).</div>
      </div>
      <div class="text-slate-400 text-[10.5px] italic pt-1">Click any metric below for an in-depth architectural breakdown:</div>
    `,
    badge: "Metric Deep Dive",
    suggestedTitle: "Metric & Architecture Deep Dives",
    suggestedIcon: "💡",
    cards: [
      { id: "explain_this_metric", icon: "📐", iconColor: "text-yellow-400", title: "Weibull Hazard Formula" },
      { id: "inspect_cas_mutex", icon: "🛡️", iconColor: "text-emerald-400", title: "Atomic CAS Mutex Logic" },
      { id: "explain_3tier", icon: "🏛️", iconColor: "text-sky-400", title: "3-Tier Isolation Boundary" },
      { id: "explain_merkle", icon: "📜", iconColor: "text-teal-400", title: "SHA-256 Merkle Ledger" },
      { id: "explain_trai", icon: "⚖️", iconColor: "text-amber-400", title: "TRAI & DPDP Guardrails" },
      { id: "explain_competitors", icon: "🏆", iconColor: "text-cyan-400", title: "vs. Stripe / Netflix" }
    ]
  },
  Explore: {
    heroHtml: `
      <div class="font-bold text-blue-400 flex items-center gap-1.5 pb-0.5">
      ...
      <div class="space-y-1 pl-1 text-[11px] pt-1">
        <div>• <strong class="text-sky-300">Live Chaos Sandbox</strong>: Real-time bank switch collapses & 5-stage recovery.</div>
        <div>• <strong class="text-purple-300">Voice AI Studio</strong>: Trilingual Hinglish, Telugu & English acoustic negotiation.</div>
        <div>• <strong class="text-emerald-300">Decision Trail</strong>: Cryptographic SHA-256 Merkle audit certificates.</div>
      </div>
      <div class="text-slate-400 text-[10.5px] italic pt-1">Click any mission module below to jump directly to it:</div>
    `,
    badge: "Platform Tour",
    suggestedTitle: "Quick Platform Navigation",
    suggestedIcon: "🧭",
    cards: [
      { id: "start_guided_tour", icon: "⚡", iconColor: "text-sky-400", title: "Live Chaos Failure Sandbox" },
      { id: "explore_voice", icon: "🎙️", iconColor: "text-purple-400", title: "Deep Loop Voice AI Studio" },
      { id: "explore_trail", icon: "📜", iconColor: "text-emerald-400", title: "SHA-256 Decision Trail" },
      { id: "explore_roi", icon: "📊", iconColor: "text-teal-400", title: "Live ROI Calculator" },
      { id: "explore_multirail", icon: "💳", iconColor: "text-cyan-400", title: "Multi-Rail Gateway Matrix" },
      { id: "explore_mobile", icon: "📱", iconColor: "text-blue-400", title: "Mobile Flutter Suite" }
    ]
  }
};
...
export function setCopilotMode(mode) {
  ...
  modes.forEach(m => {
    ...
    if (btn) {
      if (m === mode) {
        btn.className = "copilot-mode-btn px-3 py-1 rounded-full text-xs font-semibold bg-[#0c66ee] text-white shadow-md shadow-blue-500/30 transition-all cursor-pointer";
      } else {
        btn.className = "copilot-mode-btn px-2.5 py-1 rounded-full text-xs font-medium text-slate-300 hover:text-white hover:bg-white/5 transition-all cursor-pointer flex items-center gap-1";
      }
    }
  });
...
export function renderCopilotHeroCard(mode) {
  ...
  container.innerHTML = `
    <div class="flex gap-2.5 items-start">
      <div class="relative w-8 h-8 rounded-full p-0.5 bg-gradient-to-tr from-emerald-400 to-sky-400 shrink-0 shadow-sm">
        ...
        <span class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-sky-400 rounded-full border border-[#08152b]"></span>
      </div>
      <div class="flex-1 bg-[#0d2142] p-3.5 rounded-2xl rounded-tl-sm border border-sky-900/40 shadow-sm space-y-2 text-slate-200">
        <div class="flex items-center justify-between text-[10px] text-slate-400 border-b border-sky-900/30 pb-1">
          <span class="font-bold text-sky-400 flex items-center gap-1">
            <span>Razor SRE Copilot</span>
            <span class="px-1.5 py-0.2 rounded text-[8.5px] bg-[#07152b] text-sky-300 font-mono">${data.badge}</span>
          </span>
          <button onclick="speakLastCopilotMessage(this)" class="text-sky-400 hover:text-sky-300 flex items-center gap-1 font-semibold cursor-pointer">
            🔊 Speak
          </button>
        </div>
...
export function renderCopilotSuggestions(mode) {
  ...
  if (gridEl && Array.isArray(data.cards)) {
    gridEl.innerHTML = data.cards.map(card => `
      <button onclick="triggerCopilotSuggestion('${card.id}')" class="w-full px-3 py-2.5 rounded-xl bg-[#0d2244]/80 hover:bg-[#122b56] border border-sky-900/50 hover:border-sky-500/50 text-left transition-all cursor-pointer flex items-center justify-between group shadow-sm">
        <div class="flex items-center gap-2">
          <span class="text-base ${card.iconColor} group-hover:scale-110 transition-transform">${card.icon}</span>
          <span class="text-[11px] font-semibold text-slate-200 group-hover:text-white">${card.title}</span>
        </div>
        <span class="text-slate-400 group-hover:text-sky-300 group-hover:translate-x-0.5 transition-all text-sm font-bold">›</span>
      </button>
    `).join("");
  }
}
...
export function triggerCopilotSuggestion(actionKey) {
  ...
  switch (actionKey) {
    case "whats_happening_today":
      ...
      answerHtml = `
        <div class="space-y-1.5">
          <div class="text-teal-400 font-bold">Real-Time Control Plane Status (Today's Telemetry)</div>
          <div>• <strong>Total Volume Processed</strong>: <span class="text-white font-semibold">₹48,20,000</span> across 1,928 transactions.</div>
          <div>• <strong>Failures Intercepted</strong>: <span class="text-rose-300 font-semibold">142</span> (down from 260 unmanaged baseline).</div>
          <div>• <strong>Autonomously Recovered</strong>: <span class="text-emerald-400 font-bold">111 transactions (78.17% Yield)</span>.</div>
          <div>• <strong>Net Recovered GMV</strong>: <span class="text-white font-bold">₹4,25,600</span> rescued from terminal drop.</div>
          <div>• <strong>Active Rail States</strong>: HDFC (<span class="text-amber-400">Degraded 504</span>), SBI (<span class="text-emerald-400">Operational</span>), Axis (<span class="text-emerald-400">Operational</span>), PhonePe (<span class="text-emerald-400">Operational</span>).</div>
          <div>• <strong>Idempotency Collisions</strong>: <span class="text-emerald-400 font-bold">0 double-debits</span> across 50 concurrent retry threads.</div>
        </div>
      `;
      break;
    ...
    case "find_failed_payments":
      ...
      answerHtml = `
        <div class="space-y-2">
          <div class="text-sky-400 font-bold">Recent Intercepted Payment Failures</div>
          <div class="p-2 rounded-xl bg-[#08152b] border border-sky-900/40 space-y-0.5">
            <div class="flex justify-between items-center"><strong class="text-white">pay_hdfc_99812</strong> <span class="text-emerald-400 font-bold">₹85,000</span></div>
            <div class="text-[10.5px] text-slate-300">Failure: <code class="text-rose-400 font-mono">504 Gateway Timeout</code> (HDFC CBS)</div>
            <div class="text-[10px] text-cyan-300">Action: Fast-Loop shifted to Weibull peak (+45m). B2B Voice agent dispatched.</div>
          </div>
          <div class="p-2 rounded-xl bg-[#08152b] border border-sky-900/40 space-y-0.5">
            <div class="flex justify-between items-center"><strong class="text-white">pay_sbi_44109</strong> <span class="text-emerald-400 font-bold">₹3,499</span></div>
            <div class="text-[10.5px] text-slate-300">Failure: <code class="text-amber-400 font-mono">SOFT_DECLINE_INSUFFICIENT_FUNDS</code></div>
            <div class="text-[10px] text-cyan-300">Action: 1-Click WhatsApp dynamic UPI QR link dispatched.</div>
          </div>
        </div>
      `;
      break;
    ...
    case "show_revenue_at_risk":
      ...
      answerHtml = `
        <div class="space-y-1.5">
          <div class="text-amber-400 font-bold">Revenue at Risk Assessment (Current Cohort)</div>
          <div>• <strong>Total Volume at Risk</strong>: <span class="text-rose-400 font-bold">₹12,40,000</span> (12.5% merchant failure rate).</div>
          <div>• <strong>High-Ticket B2B Invoices (&gt;₹50,000)</strong>: <span class="text-white font-semibold">₹8,15,000</span> (65.7% of risk).</div>
          <div>• <strong>Consumer Soft Declines (&lt;₹10,000)</strong>: <span class="text-white font-semibold">₹4,25,000</span> (34.3% of risk).</div>
          <div>• <strong>Projected Salvageable GMV</strong>: <span class="text-emerald-400 font-bold">₹9,72,000</span> using OmniRevive Multi-Rail.</div>
          <div>• <strong>Merchant Margin Protected</strong>: <span class="text-white font-semibold">₹1,94,400</span> (at 200 bps standard take rate).</div>
        </div>
      `;
      break;
    ...
    case "analyze_recovery_rate":
      ...
      answerHtml = `
        <div class="space-y-1.5">
          <div class="text-cyan-400 font-bold">Cohort Recovery Rate Empirical Analysis</div>
          <div>• <strong>OmniRevive-OS Yield</strong>: <span class="text-emerald-400 font-extrabold text-sm">78.39%</span></div>
          <div>• <strong>Industry Static Retry Baseline</strong>: <span class="text-slate-400 font-semibold">42.24%</span></div>
          <div>• <strong>Net Recovery Lift</strong>: <span class="text-cyan-300 font-bold">+36.15% Absolute Gain</span></div>
    ...
    case "explain_weibull":
      ...
      answerHtml = `
        <div class="space-y-1.5">
          <div class="text-yellow-400 font-bold">Weibull Hazard Function h(t)</div>
          <div class="font-mono text-cyan-300 text-[11px] bg-[#071328] p-2 rounded-lg border border-sky-900/50">
            h(t) = (k / λ) * (t / λ)^(k - 1)
          </div>
    ...
    case "explore_sandbox":
      ...
      answerHtml = `
        <div class="space-y-2">
          <div class="text-blue-400 font-bold">Live Chaos Failure Sandbox</div>
    ...
    case "explore_voice":
      ...
      answerHtml = `
        <div class="space-y-2">
          <div class="text-purple-400 font-bold">Deep Loop Voice AI Studio</div>
          <div class="text-slate-200 leading-relaxed">
            Test acoustic negotiation with native voices across Indian languages:
          </div>
          <div class="flex flex-wrap gap-1.5 text-[10.5px]">
            <span class="px-2 py-0.5 rounded bg-purple-950/60 text-purple-300 border border-purple-500/40">Telugu (te-IN)</span>
            <span class="px-2 py-0.5 rounded bg-amber-950/60 text-amber-300 border border-amber-500/40">Hindi (hi-IN)</span>
            <span class="px-2 py-0.5 rounded bg-blue-950/60 text-blue-300 border border-blue-500/40">English (en-IN)</span>
          </div>
          <div class="pt-1">
    ...
    case "explain_merkle":
      ...
      answerHtml = `
        <div class="space-y-2">
          <div class="text-emerald-400 font-bold">SHA-256 Merkle Decision Trail</div>
          <div class="text-slate-200 leading-relaxed">
            Every recovery action is committed to an immutable hash chain:
          </div>
          <div class="font-mono text-[10px] text-cyan-300 bg-[#071328] p-2 rounded-lg border border-sky-900/50">
    ...
    case "explore_roi":
      ...
      answerHtml = `
        <div class="space-y-2">
          <div class="text-teal-400 font-bold">Revenue Recovery Calculator</div>
          <div class="text-slate-200 leading-relaxed">
    ...
    case "inspect_cas_mutex":
      ...
      answerHtml = `
        <div class="space-y-1.5">
          <div class="text-emerald-400 font-bold">Zero Double-Debit Concurrency Protection</div>
          <div>• <strong>Compare-And-Swap (CAS)</strong>: Acquired in <span class="font-mono text-cyan-300">0.23ms</span> on <code>merchant_id:payment_id</code>.</div>
          ...
          <div>• <strong>Result</strong>: Exactly <span class="text-emerald-400 font-bold">0 double debits</span> and 0 idempotency violations.</div>
        </div>
      `;
      break;
    ...
    case "trace_pay_hdfc":
      ...
      answerHtml = `
        <div class="space-y-1.5">
          <div class="text-sky-400 font-bold">Transaction Trace: pay_hdfc_99812 (₹85,000)</div>
          ...
          <div>• <strong>Status</strong>: <span class="px-1.5 py-0.5 rounded bg-emerald-950/60 text-emerald-400 font-bold border border-emerald-500/40">MUTEX_LOCKED</span></div>
        </div>
      `;
      break;
...
export function renderFocusedHeroCard(title, bodyHtml) {
  ...
  container.innerHTML = `
    <div class="flex gap-2.5 items-start">
      <div class="relative w-8 h-8 rounded-full p-0.5 bg-gradient-to-tr from-emerald-400 to-sky-400 shrink-0 shadow-sm">
        <img src="copilot_avatar.jpg" onerror="this.onerror=null; this.src='assets/copilot_avatar.jpg'; if(!this.src) this.src='/copilot_avatar.jpg';" alt="Avatar" class="w-full h-full rounded-full object-cover">
        <span class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border border-[#08152b]"></span>
      </div>
      <div class="flex-1 bg-[#0d2142] p-3.5 rounded-2xl rounded-tl-sm border border-sky-900/40 shadow-sm space-y-2.5 text-slate-200">
        <div class="flex items-center justify-between text-[10px] text-slate-400 border-b border-sky-900/40 pb-1.5">
          <span class="font-bold text-sky-400 flex items-center gap-1 truncate max-w-[240px]">
            ${title}
          </span>
          <div class="flex items-center gap-2 shrink-0">
            <button onclick="speakLastCopilotMessage(this)" class="text-sky-400 hover:text-sky-300 font-semibold cursor-pointer">
              🔊 Speak
            </button>
            <button onclick="resetCopilotHeroCard()" class="px-2 py-0.5 rounded-md bg-[#091830] hover:bg-[#12284d] text-slate-300 hover:text-white border border-sky-900/60 font-semibold transition cursor-pointer">
              ↩ Back
            </button>
          </div>
        </div>
...
export async function handleCopilotCustomQuery(e) {
  ...
  container.innerHTML = `
    <div class="flex gap-2.5 items-start">
      ...
      <div class="bg-[#0d2142] p-3 px-4 rounded-2xl rounded-tl-sm border border-sky-900/40 text-[11px] text-sky-400 font-medium animate-pulse flex items-center gap-2">
